...
income_condition – 소득 기준 (중위소득 n% 이하 등)
...
프론트는 이 응답으로 “(AI) 지금 이렇게 이해했어요” 요약 카드와
[수정/추가하기] 미니 폼을 렌더링한다.
...
청년 버스카드(교통비), 어학시험 응시료 지원, 월세·보증금 지원, 청년 저축지원 같은 정책을
policies 테이블에 upsert.

처리 흐름

여러 페이지에 걸친 정책 목록 GET

개별 정책 상세 정보 GET

DB에 저장 (없으면 insert, 있으면 update)
...
역할
policies.raw_text 또는 title + summary를 chunk로 나누고
...
policy_id, chunk_index, embedding 저장
...
message + parsedProfile을 하나의 텍스트로 합쳐 Embedding 생성

pgvector의 cosine_distance 등을 사용해 policy_embeddings에서 Top-N 검색

나이/지역/카테고리(교육/어학, 교통, 주거 등) 매칭으로 룰 기반 점수 추가
...
policies.raw_text + 관련 텍스트를 RAG 컨텍스트로 구성

OpenAI Chat API 호출 시,

“공고문에 근거가 있는 내용만 답변하고
없으면 ‘공고문에 명시되어 있지 않다’고 말하기”라는 시스템 프롬프트 적용
...
정책 텍스트 + profile을 기반으로 OpenAI Chat API 호출

“지원 동기”, “활용 계획”을 각각 400~800자 / 300~600자 정도 길이로 생성

drafts 테이블에 저장 후, 프론트로 반환
...
상단: 서비스 간단 소개 (청년 버스카드·어학시험·월세 지원 등)
...
[수정/추가하기] 버튼 클릭 시 모달 폼 오픈 → 브라우저 상태 또는 /api/profile/update로 저장

정책 추천 리스트

/api/policies/recommend 결과를 정책 카드 리스트로 보여줌
...
LLM이 생성한 지원 동기/활용 계획 문단을 에디터에 띄우고, 사용자가 직접 수정
...
사용되는 공공 데이터 / OPEN API는 각 제공 기관의 이용 약관 및 표기 의무를 반드시 준수해야 합니다.

이 레포지토리의 소스 코드 라이선스(MIT 등)는 팀에서 별도로 지정합니다.
